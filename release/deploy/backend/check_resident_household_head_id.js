...
db.all('PRAGMA table_info(residents)', (err, rows) => {
  ...
  if (!hasHouseholdHeadId) {
    ...
    console.log(
      `${householdHeadIdField.cid}: ${householdHeadIdField.name} (${householdHeadIdField.type}) - ${householdHeadIdField.notnull ? 'NOT NULL' : 'NULL'} - 默认值: ${householdHeadIdField.dflt_value}`
    )
