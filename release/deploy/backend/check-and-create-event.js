...
const db = new sqlite3.Database('app.db', (err) => {
  ...
  db.get(
    ...
    (err, row) => {
      ...
      if (row) {
        console.log('event_reminders 表已存在')

        // 插入示例数据
        const insertSql = `INSERT OR IGNORE INTO event_reminders (event_type, title, event_date, event_time, location, description) VALUES
        ...
        db.run(insertSql, function (err) {
          ...
          db.all('SELECT * FROM event_reminders', (err, rows) => {
            if (err) {
              ...
              console.log(`event_reminders 表中当前有 ${rows.length} 条记录`)
              rows.forEach((r) => {
        ...
        console.log('event_reminders 表不存在，正在创建...')

        const createSql = `CREATE TABLE event_reminders (
