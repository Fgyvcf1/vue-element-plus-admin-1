  ...
  try {
    ...
    db.all(`SELECT id, household_id FROM residents`, (err, residents) => {
      ...
      const updateResident = (index) => {
        ...
        db.get(
          `SELECT household_number FROM households WHERE id = ?`,
          [oldHouseholdId],
          (err, household) => {
            if (err) {
            ...
            if (!household) {
              console.error(
                `居民ID ${residentId} 对应的家庭不存在 (household_id: ${oldHouseholdId})`
              )
              errorCount++
              updateResident(index + 1)
              return
            ...
            console.log(
              `居民ID ${residentId}: 将household_id从 ${oldHouseholdId} 更新为 ${newHouseholdId}`
            )

            // 4. 更新居民记录的household_id
            db.run(
              `UPDATE residents SET household_id = ? WHERE id = ?`,
              [newHouseholdId, residentId],
              (err) => {
                if (err) {
