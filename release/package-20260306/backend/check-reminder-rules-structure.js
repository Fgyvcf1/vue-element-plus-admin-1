...
db.get('PRAGMA table_info(reminder_rules)', (err, columns) => {
  ...
  columns.forEach((col) => {
    console.log(
      `${col.name}\t\t${col.type}\t\t${col.notnull ? '是' : '否'}\t${col.pk ? '是' : '否'}`
    )
  })
  ...
  db.all('SELECT * FROM reminder_rules', (err, rows) => {
    if (err) {
      ...
      if (rows.length === 0) {
        ...
        rows.forEach((row) => {
          console.log(
            `${row.id}\t${row.rule_type}\t\t${row.rule_name}\t${row.rule_value}\t${row.description || ''}\t${row.status}\t${row.created_at}\t${row.updated_at}`
          )
        })
      }
    }
