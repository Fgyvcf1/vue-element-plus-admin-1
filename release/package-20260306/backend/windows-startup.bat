...
if not defined DB_PASSWORD (
    ...
    for /f "usebackq delims=" %%p in (`powershell -NoProfile -Command "$p = Read-Host '请输入数据库密码（不会回显）' -AsSecureString; [Runtime.InteropServices.Marshal]::PtrToStringAuto([Runtime.InteropServices.Marshal]::SecureStringToBSTR($p))"`) do endlocal & set "DB_PASSWORD=%%p"
)
if "%DB_PASSWORD%"=="" (
...
const isDeployEnv = appEnv === 'production' || appEnv === 'prod' || process.env.DEPLOY_ENV === '1' || process.env.DEPLOY === '1';
const hasEnvPassword = Object.prototype.hasOwnProperty.call(process.env, 'DB_PASSWORD');

if (isDeployEnv && !hasEnvPassword) {
  console.error('❌ 部署环境必须通过环境变量 DB_PASSWORD 提供数据库密码');
  process.exit(1);
}

const dbPassword = hasEnvPassword
  ? process.env.DB_PASSWORD
  : (isDeployEnv ? '' : (fileConfig.password || 'strongpass791002'));
