...
REM Deployment environment marker (forces env var password)
set "APP_ENV=production"
set "DEPLOY=1"

REM DB password must be provided via environment variable
if not defined DB_PASSWORD (
    for /f "usebackq delims=" %%p in (`powershell -NoProfile -Command "$p = Read-Host 'Enter database password (hidden)' -AsSecureString; [Runtime.InteropServices.Marshal]::PtrToStringAuto([Runtime.InteropServices.Marshal]::SecureStringToBSTR($p))"`) do set "DB_PASSWORD=%%p"
)
if "%DB_PASSWORD%"=="" (
    echo ❌ DB_PASSWORD is required to continue
    pause
    exit /b 1
)
