...
// 先检查是否已有相关数据
const checkSql =
  'SELECT COUNT(*) as count FROM dictionaries WHERE category = "relationship_to_head"'
  ...
  relationships.forEach((relationship, index) => {
    ...
    db.run(insertSql, params, function (err) {
      ...
      if (insertedCount === relationships.length) {
        ...
        // 验证插入结果
        db.all(
          'SELECT id, category, value, display_order FROM dictionaries WHERE category = "relationship_to_head" ORDER BY display_order ASC',
          (err, rows) => {
            if (err) {
            ...
            rows.forEach((row) => {
              console.log(
                `id: ${row.id}, category: ${row.category}, value: ${row.value}, display_order: ${row.display_order}`
              )
            })
