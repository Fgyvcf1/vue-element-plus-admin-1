...
function checkHouseholdIdCardField() {
  ...
  db.all('PRAGMA table_info(households)', (err, columns) => {
    ...
    if (hasIdCardField) {
      ...
      db.all(
        'SELECT id, household_number, household_head_name, household_head_id_card FROM households LIMIT 5',
        (err, rows) => {
          if (err) {
          ...
          rows.forEach((row) => {
            console.log(
              `  ID: ${row.id}, 户号: ${row.household_number}, 户主姓名: ${row.household_head_name}, 身份证号: ${row.household_head_id_card || '空'}`
            )
          })

          // 查询有多少条记录的household_head_id_card字段是空的
          db.get(
            "SELECT COUNT(*) as emptyCount FROM households WHERE household_head_id_card IS NULL OR household_head_id_card = ''",
            (err, result) => {
              if (err) {
