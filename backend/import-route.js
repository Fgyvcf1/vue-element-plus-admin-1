...
router.post('/import-residents', async (req, res) => {
  ...
  try {
    ...
    // 只输出关键统计信息
    console.log('数据解析完成 - 户主:', headOfHouseholdRows.length, '人, 家庭成员:', familyMemberRows.length, '人, 错误:', parseErrors.length, '个');
    ...
    const [existingResidents] = await db.pool.execute(`SELECT id, id_card, household_id FROM residents WHERE id_card IN (${allIdCards.map(() => '?').join(',')})`, allIdCards);
    ...
    for (const rowData of existingHeads) {
      ...
      // 查询对应的户编号
      const [householdRows] = await db.pool.execute('SELECT household_number FROM households WHERE id = ? OR household_number = ?', 
      ...
      if (householdRows.length > 0) {
        ...
        // 生成户编号
        householdNumber = await generateUniqueHouseholdId(villageGroup, idCard, checkHouseholdNumberExists);
        
        // 插入家庭数据
        const householdColumns = ['household_number', 'household_head_name', 'household_head_id_card', 'village_group', 'address', 'phone_number', 'ethnicity', 'gender', 'status', 'registered_date', 'household_type', 'housing_type'];
        ...
        await db.pool.execute('UPDATE residents SET household_id = ? WHERE id = ?', [householdNumber, existingResident.id]);
    ...
    if (newHeads.length > 0) {
      ...
      for (const rowData of newHeads) {
        ...
        // 生成户主ID
        const householdNumber = await generateUniqueHouseholdId(villageGroup, idCard, checkHouseholdNumberExists);
    ...
    const newFamilyMembers = familyMemberRows.filter(row => !existingIdCardMap.has(row.residentData.id_card));
    ...
    for (const rowData of existingFamilyMembers) {
      ...
      const memberAddress = householdData.address || residentData.address || residentData.Home_address || '';
    ...
    if (newFamilyMembers.length > 0) {
      ...
      const validNewMembers = newFamilyMembers.filter(rowData => {
        ...
        if (!householdId) {
          console.log('无效家庭成员（找不到户主）:', rowData.residentData.name, '户主:', householdHeadName);
          return false;
        }
      ...
      if (validNewMembers.length > 0) {
        ...
        const residentValues = validNewMembers.map(rowData => {
          ...
          console.log('插入新家庭成员:', residentData.name, '到户主:', householdHeadName, '户编号:', householdId);
          return [
            householdId,
            residentData.name || '',
            residentData.gender || '',
