...
// 模拟导入数据
const importData = {
  headers: ['序号', '姓名', '户主姓名', '与户主关系', '性别', '民族', '组别', '户籍详细地址', '出生日期', '身份证号码', '年龄', '联系方式'],
  data: [
...
async function testImport() {
  try {
    ...
    // 检查倪清春是否已存在
    const [existing] = await db.pool.execute('SELECT id, household_id FROM residents WHERE id_card = ?', ['45242519630408121X']);
    ...
    for (const rowData of headOfHouseholdRows) {
      ...
      if (existing.length > 0) {
        // 已存在，检查 households 表
        const [householdRows] = await db.pool.execute('SELECT household_number FROM households WHERE household_number = ?', [existing[0].household_id]);
        ...
        if (householdRows.length === 0) {
          ...
          // 更新 residents
          await db.pool.execute('UPDATE residents SET household_id = ? WHERE id = ?', [householdNumber, existing[0].id]);
