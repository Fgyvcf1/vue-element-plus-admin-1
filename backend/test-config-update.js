...
async function testConfigUpdate() {
  try {
    ...
    if (rows1.length > 0) {
      console.log(`   ${rows1[0].config_key}: ${rows1[0].config_value} (类型: ${rows1[0].value_type})`);
    } else {
    ...
    if (rows2.length > 0) {
      console.log(`   ${rows2[0].config_key}: ${rows2[0].config_value} (类型: ${rows2[0].value_type})`);
    }
    ...
    if (rows3.length > 0) {
      console.log(`   ${rows3[0].config_key}: ${rows3[0].config_value} (类型: ${rows3[0].value_type})`);
    }
