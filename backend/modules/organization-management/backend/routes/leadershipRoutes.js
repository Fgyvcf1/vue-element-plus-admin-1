...
router.get('/committee-members', (req, res) => {
  ...
  let sql = `SELECT cm.*, r.name, r.gender, r.phone_number, r.id_card, r.date_of_birth, r.address
...
router.get('/committee-members/:id', (req, res) => {
  ...
  const sql = `SELECT cm.*, r.name, r.gender, r.phone_number, r.id_card, r.date_of_birth, r.address
             FROM committee_members cm
             JOIN residents r ON cm.resident_id = r.id
             WHERE cm.id = ?`;
...
// 添加成员
router.post('/committee-members', (req, res) => {
  ...
  const params = [
    ...
    termEndDateValue, position, status || 'current', remarks
  ];
