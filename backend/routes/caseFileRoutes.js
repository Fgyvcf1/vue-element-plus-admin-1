...
router.get('/:id/view', async (req, res) => {
  ...
  try {
    ...
    res.setHeader('Content-Disposition', 'inline; filename="' + encodeURIComponent(caseFile.file_name) + '"');
...
router.get('/:id/download', async (req, res) => {
  ...
  try {
    ...
    // 设置响应头，让浏览器下载PDF
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', 'attachment; filename="' + encodeURIComponent(caseFile.file_name) + '"');
