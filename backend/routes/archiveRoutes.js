...
router.get('/archives', async (req, res) => {
  ...
  try {
    ...
    if (keyword) { sql += ' AND a.archive_id LIKE ?'; params.push(`%${keyword}%`); }
    ...
    if (keyword) { countSql += ' AND archive_id LIKE ?'; countParams.push(`%${keyword}%`); }
    ...
    const formattedRows = rows.map(row => {
      ...
      return { ...row, applicant_display: formatPeople(row.applicant_names, row.applicant_count), respondent_display: formatPeople(row.respondent_names, row.respondent_count) };
    });
...
router.get('/archives/prefixes', async (req, res) => {
  try {
    const [rows] = await db.pool.execute('SELECT prefix, current_number FROM archive_sequences ORDER BY prefix');
...
router.post('/archives/prefixes', async (req, res) => {
  ...
  try {
    const [rows] = await db.pool.execute('SELECT id FROM archive_sequences WHERE prefix = ?', [prefix]);
    if (rows.length > 0) return res.status(400).json({ code: 400, message: '前缀已存在' });

    await db.pool.execute('INSERT INTO archive_sequences (prefix, current_number, created_at, updated_at) VALUES (?, ?, NOW(), NOW())', [prefix, 0]);
...
router.delete('/archives/prefixes/:prefix', async (req, res) => {
  ...
  try {
    const [rows] = await db.pool.execute('SELECT id FROM mediation_archives WHERE prefix = ?', [prefix]);
    if (rows.length > 0) return res.status(400).json({ code: 400, message: '该前缀已被档案使用，无法删除' });
...
router.post('/archives', async (req, res) => {
  ...
  try {
    ...
    const [seqRows] = await connection.execute('SELECT current_number FROM archive_sequences WHERE prefix = ?', [prefix]);
    if (seqRows[0]) {
      await connection.execute('UPDATE archive_sequences SET current_number = ?, updated_at = NOW() WHERE prefix = ?', [newNumber, prefix]);
    } else {
      await connection.execute('INSERT INTO archive_sequences (prefix, current_number, created_at, updated_at) VALUES (?, ?, NOW(), NOW())', [prefix, newNumber]);
    }

    // 获取档案表最大ID并生成新ID
    const [maxArchiveIdResult] = await connection.execute('SELECT MAX(id) as maxId FROM mediation_archives');
    const newArchiveId = (maxArchiveIdResult[0].maxId || 0) + 1;

    // 创建档案
    await connection.execute('INSERT INTO mediation_archives (id, archive_id, prefix, sequence_number, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, NOW(), NOW())', [newArchiveId, archiveId, prefix, newNumber, 'draft']);

    await connection.commit();
    res.json({ code: 20000, message: '档案创建成功', data: { archive_id: archiveId, prefix, sequence_number: newNumber, status: 'draft' } });
  } catch (err) {
...
router.get('/archives/:id', async (req, res) => {
  ...
  try {
    // 获取档案信息
    const [archiveRows] = await db.pool.execute('SELECT * FROM mediation_archives WHERE archive_id = ?', [id]);
    if (archiveRows.length === 0) return res.status(404).json({ code: 404, message: '档案不存在' });
    const archive = archiveRows[0];

    // 获取申请书信息
    const [applicationRows] = await db.pool.execute('SELECT * FROM mediation_applications WHERE archive_id = ?', [id]);
    ...
    // 获取调解记录
    const [recordRows] = await db.pool.execute('SELECT * FROM mediation_records WHERE archive_id = ? ORDER BY mediation_date DESC', [id]);

    // 获取调解协议
    const [agreementRows] = await db.pool.execute('SELECT * FROM mediation_agreements WHERE archive_id = ?', [id]);
    const agreement = agreementRows[0] || null;

    // 获取附件
    const [attachmentRows] = await db.pool.execute('SELECT * FROM archive_attachments WHERE archive_id = ? ORDER BY created_at', [id]);
...
router.post('/archives/:id/application', async (req, res) => {
  const { id } = req.params;
  const { dispute_type, dispute_description, request_content, occurrence_date, occurrence_location, apply_date, applicants, respondents } = req.body;

  console.log('收到保存申请书请求:', { id, dispute_type, occurrence_date, occurrence_location, apply_date });
  ...
  try {
    ...
    // 获取申请书最大ID并生成新ID
    const [maxAppIdResult] = await connection.execute('SELECT MAX(id) as maxId FROM mediation_applications');
    ...
    await connection.execute(
      `INSERT INTO mediation_applications (id, archive_id, dispute_type, dispute_description, request_content, occurrence_date, occurrence_location, apply_date, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
      [newAppId, id, dispute_type || '', dispute_description || '', request_content || '', occurrence_date || null, occurrence_location || null, currentDate]
    );

    // 插入申请人（包含新字段）
    if (applicants && applicants.length > 0) {
      // 获取最大ID
      const [maxIdResult] = await connection.execute('SELECT MAX(id) as maxId FROM mediation_applicants');
      ...
      for (const item of applicants) {
        ...
        await connection.execute(
          `INSERT INTO mediation_applicants (id, archive_id, is_resident, resident_id, name, id_card, phone, address, 
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
          [maxId, id, isResident ? 1 : 0, residentId, 
           item.name || '', item.idCard || null, item.phone || null, item.address || null,
           item.gender || null, item.ethnicity || '汉族', item.age || null, item.occupation || null]
        );
      }
    }
    ...
    if (respondents && respondents.length > 0) {
      // 获取最大ID
      const [maxIdResult] = await connection.execute('SELECT MAX(id) as maxId FROM mediation_respondents');
      let maxId = maxIdResult[0].maxId || 0;
    ...
    // 更新档案状态
    await connection.execute('UPDATE mediation_archives SET status = ?, updated_at = NOW() WHERE archive_id = ?', ['in_progress', id]);
...
router.post('/archives/:id/records', async (req, res) => {
  const { id } = req.params;
  const { mediation_date, mediation_location, mediators, process_record, mediation_result, agreement } = req.body;
  ...
  try {
    ...
    await connection.execute(
      `INSERT INTO mediation_records (id, archive_id, mediation_date, mediation_location, mediators, process_record, mediation_result, agreement, created_at) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
      [newId, id, mediation_date, mediation_location, mediators, process_record, mediation_result, agreement]
    );
...
router.post('/archives/:id/agreement', async (req, res) => {
  const { id } = req.params;
  const { agreement_date, agreement_content, performance_period, breach_liability, party_a_sign, party_b_sign, mediator_sign } = req.body;
  ...
  try {
    ...
    // 获取最大ID并生成新ID
    const [maxIdResult] = await connection.execute('SELECT MAX(id) as maxId FROM mediation_agreements');
    const newId = (maxIdResult[0].maxId || 0) + 1;
    ...
    await connection.execute(
      `INSERT INTO mediation_agreements (id, archive_id, agreement_date, agreement_content, performance_period, breach_liability, party_a_sign, party_b_sign, mediator_sign, created_at) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
      [newId, id, agreement_date, agreement_content, performance_period, breach_liability, party_a_sign, party_b_sign, mediator_sign]
    );
...
router.get('/archives/:id/files', async (req, res) => {
  ...
  try {
    const [rows] = await db.pool.execute('SELECT * FROM archive_files WHERE archive_id = ? ORDER BY created_at', [id]);
    res.json({ code: 20000, data: rows });
  } catch (err) {
...
router.get('/archives/:id/attachments', async (req, res) => {
  ...
  try {
    const [rows] = await db.pool.execute('SELECT * FROM archive_attachments WHERE archive_id = ? ORDER BY created_at', [id]);
    res.json({ code: 20000, data: rows });
  } catch (err) {
...
router.post('/archives/:id/attachments', archiveUpload.array('file', 10), async (req, res) => {
  ...
  try {
    ...
    // 获取最大ID
    const [maxIdResult] = await connection.execute('SELECT MAX(id) as maxId FROM archive_attachments');
    let maxId = maxIdResult[0].maxId || 0;
...
router.delete('/archives/:id/attachments/:attachmentId', async (req, res) => {
  ...
  try {
    ...
    // 先获取附件信息，用于删除文件
    const [rows] = await connection.execute('SELECT * FROM archive_attachments WHERE id = ? AND archive_id = ?', [attachmentId, id]);
    ...
    // 删除附件记录
    await connection.execute('DELETE FROM archive_attachments WHERE id = ? AND archive_id = ?', [attachmentId, id]);
