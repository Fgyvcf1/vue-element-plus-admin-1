...
router.get('/committee-members', checkPermission('organization:view'), async (req, res) => {
  ...
  let listSql = `SELECT cm.*, r.name, r.gender, r.phone_number, r.id_card, r.ethnicity, r.political_status, DATE_FORMAT(r.date_of_birth, '%Y-%m-%d') AS date_of_birth, r.\`Home_address\` AS address
                 ${filterSql}
...
router.get('/committee-members/history', checkPermission('organization:view'), async (req, res) => {
  ...
  const sql = `SELECT cm.*, r.name, r.gender, r.phone_number, r.id_card, r.ethnicity, r.political_status, DATE_FORMAT(r.date_of_birth, '%Y-%m-%d') AS date_of_birth, r.\`Home_address\` AS address
