...
// 查询所有家庭信息
db.all('SELECT id, household_head_name, household_number, address FROM households', [], (err, rows) => {
  if (err) {
  ...
  rows.forEach(row => {
    console.log(`ID: ${row.id}, 户主: ${row.household_head_name}, 户号: ${row.household_number}, 地址: ${row.address}`);
  });
