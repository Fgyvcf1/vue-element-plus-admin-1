...
**问题**: 代码使用了`is_read`、`updated_at`字段，但表中没有这些字段
**修复**: 已将INSERT语句改为使用正确的字段：
...
**原因**: 表不存在
**解决**: 运行 `node create-notification-tables.js`

### 错误2: "table notifications has no column named is_read"
**原因**: 字段名错误
**解决**: 代码已修复，使用`status`字段
