...
// 查询姓名为张三的居民
db.all(
  'SELECT id, name, household_id FROM residents WHERE name LIKE ?',
  ['%张三%'],
  (err, rows) => {
    if (err) {
    ...
    if (rows.length > 0) {
      ...
      db.get(
        'SELECT id, household_head_name, household_number, address FROM households WHERE id = ?',
        [householdId],
        (err, household) => {
          if (err) {
          ...
          if (household) {
            console.log(
              `家庭信息: ID: ${household.id}, 户主: ${household.household_head_name}, 户号: ${household.household_number}, 地址: ${household.address}`
            )
          } else {
