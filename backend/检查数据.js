...
function checkCommitteeMembersData() {
  return new Promise((resolve, reject) => {
    ...
    db.all(
      'SELECT id, resident_id, organization_type, term_number, position FROM committee_members LIMIT 5',
      (err, rows) => {
        if (err) {
          console.error('❌ 查询失败:', err.message)
          reject(err)
...
function checkResidentsAssociation() {
  return new Promise((resolve, reject) => {
    ...
    db.all(
      'SELECT cm.id, cm.resident_id, r.id as resident_exists FROM committee_members cm LEFT JOIN residents r ON cm.resident_id = r.id WHERE r.id IS NULL LIMIT 10',
      (err, rows) => {
        if (err) {
          console.error('❌ 查询失败:', err.message)
          reject(err)
...
function checkLeftJoinQuery() {
  return new Promise((resolve, reject) => {
    ...
    db.all(
      'SELECT cm.*, r.name, r.gender, r.phone_number FROM committee_members cm LEFT JOIN residents r ON cm.resident_id = r.id WHERE cm.organization_type = ? LIMIT 5',
      ['branch_committee'],
      (err, rows) => {
        if (err) {
          console.error('❌ 查询失败:', err.message)
