    ...
    db.all(sql, [residentId], (err, rows) => {
      if (err) {
        ...
        rows.forEach((row, index) => {
          console.log(`${index + 1}. ID: ${row.id}, 类型: ${row.change_type}, 日期: ${row.change_date}, 原因: ${row.change_reason}`);
        });
        resolve(rows);
      }
    });
    ...
    await new Promise((resolve, reject) => {
      ...
      db.all(sql, [], (err, rows) => {
        if (err) {
          ...
          console.log(`查询到 ${rows.length} 条记录:`);
          rows.forEach((row, index) => {
            console.log(`${index + 1}. ID: ${row.id}, 居民ID: ${row.resident_id}, 类型: ${row.change_type}, 日期: ${row.change_date}`);
          });
          resolve();
        }
      });
