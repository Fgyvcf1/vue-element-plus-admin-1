...
db.all(
  "SELECT * FROM reminder_rules WHERE rule_type = 'age' AND status = 'active'",
  (err, rules) => {
    if (err) {
  ...
  `
    ...
    db.all(sql, [], (err, residents) => {
      ...
      residents.forEach((resident) => {
        ...
        console.log(
          `${resident.name} (${resident.id_card.substring(0, 6)}...${resident.id_card.substring(14)})`
        )
        ...
        rules.forEach((rule) => {
          ...
          // 提前提醒逻辑
          if (
            currentAge === targetAge - 1 &&
            daysUntilBirthday <= reminderDays &&
            daysUntilBirthday >= 0
          ) {
