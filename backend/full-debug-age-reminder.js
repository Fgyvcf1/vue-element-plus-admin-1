  ...
  db.all(sql, [], (err, residents) => {
    ...
    db.all("SELECT * FROM reminder_rules WHERE rule_type = 'age' AND status = 'active' AND rule_value = '60'", (err, rules) => {
      ...
      reminders.forEach((reminder, index) => {
        ...
        db.get(checkSql, [`%${reminder.resident_name}%`, reminder.reminder_content], (checkErr, existing) => {
          if (checkErr) {
          ...
          if (!existing) {
            ...
            console.log(`⏭️  通知已存在（7天内），跳过: ${reminder.resident_name} ${reminder.rule_name}`);
          }
              ...
              db.run(insertSql, params, function(insertErr) {
                ...
                if (createdCount === newReminders.length) {
                  ...
                  if (createdCount > 0) {
                    ...
                    console.log(`   1. 数据库: SELECT * FROM notifications WHERE id >= (SELECT max(id) - ${createdCount} + 1 FROM notifications)`);
